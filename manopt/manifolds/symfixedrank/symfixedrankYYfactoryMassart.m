...
%
%   November 2, 2016 (E. Massart):
%       added a definition for the Log_{Y0}(Y1)
%
...
% 	M.tangent = M.proj;
 ...
 %%%%%%%%%%%%%%
    ...
    warning('manopt:symfixedrankYYfactory:exp', ...
        ['Exponential and logarithm for symmetric, fixed-rank ' ...
        'manifold not implemented yet. Used retraction and its reciprocal instead.']);

    M.log = @logarithm; 
    function eta = logarithm(Y0, Y1) 
        [U,S,V] = svd(Y0'*Y1);               % such that A = U*S*V'
        Q = U*V';                       % Q issue from the polar decomposition A = HQ with  H = U*S*U';
        eta = Y1*Q'-Y0;
    end

    M.orth_factor = @orth_factoriz; 
    function Q = orth_factoriz(Y0, Y1) 
        [U,S,V] = svd(Y0'*Y1);               % such that A = U*S*V'
        Q = U*V';                       % Q issue from the polar decomposition A = HQ with  H = U*S*U';
    end
 %%%%%%%%%%%%%%%   
