function pb = curve_reconstruction_double_bezier_c1(varargin)
% Reconstructs the Bezier curve based on a tensorization approach.
%
% function pb = curve_reconstruction_double_bezier(pb,X,Y)
%    Reconstructs the Bezier curve based on the control points stored in
%    the structure pb, at times (t1,t2). The piecewize Bezier surface is
%    piecewise cubic and of type II.
%
% function pb = curve_reconstruction_double_bezier(pb,X,Y,'direction',DIRECTION)
%    Reconstructs the Bezier curve by applying first the given DIRECTION
%    and then the other one. Possible choices of directions are
%    'x-direction' and 'y-direction'. By default, 'x-direction' is
%    preferred.
%
...
% Change log:
% 	Feb. 20, 2019 (PYG) - Integration of the Manopt framework.
...
% see <http://www.gnu.org/licenses/>.
  ...
  % For each value of (X,Y)
    ...
    % control points
		...
		% x
		switch patchX + 1
		case 1   ; ind_x = [shiftX + 1: shiftX + deg , shiftX + deg + 2]; 
		case m-1 ; ind_x = [shiftX , shiftX + 2: shiftX + deg + 1 ]; 
		otherwise; ind_x = [shiftX , shiftX + 2: shiftX + deg , shiftX + deg + 2];
		end
		% y
		switch patchY + 1 
		case 1   ; ind_y = [shiftY + 1: shiftY + deg , shiftY + deg + 2];
		case n-1 ; ind_y = [shiftY , shiftY + 2: shiftY + deg + 1];
		otherwise; ind_y = [shiftY , shiftY + 2: shiftY + deg , shiftY + deg + 2];
		end
		
		% if only one patch
		if n == 2 ; ind_y = 1:deg+1; end
		if m == 2;  ind_x = 1:deg+1; end
		
		% reduction		
		cp = controlPoints(ind_x,ind_y);
    
    
    % Curve reconstruction -- Specific for this method
		y = decasteljau2d_tensor_c1(M,cp,t1,t2,patchX+1,patchY+1,m-1,n-1,'direction',direction);
		curve(k,l,:,:) = y;
    % end specific
		...
		waitbar(perc,h,sprintf(...
			['Processing reconstruction algorithm. Complete: %.f %%.'],...
			perc*100));
	end
	end
