...
% C^2. The method uses a double tensorization instead of a direct 2D
% approach.
%
% function pb = control_points_double_tensorization(pb)
...
% see <http://www.gnu.org/licenses/>.
  ...
  [r,c] 	    = size(dataPoints{1,1});
	...
	for i = 1:m
		bRows(i,:) = simple_tensorization(M,dataPoints(i,:), An, threshold);
	end
	...
	for j = 1:3*pb.patchN + 1
		b(:,j) = simple_tensorization(M,bRows(:,j), Am, threshold);
	end
